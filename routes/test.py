...
def embed_subtitle(input_video, subtitle_file, output_video,font_size,color,font_name):
    """
    使用FFmpeg硬嵌入SRT字幕到视频
    ...
    cmd = [
        ...
        '-vf', f"subtitles={subtitle_file}:force_style="
               "'Alignment=0,"  # 位置设置(2=居中)(0=左对齐, 2=居中, 3=右对齐)
               "Bold=0,"  # 粗体(1=启用)
               "Italic=0,"  # 斜体(1=启用)
               "Underline=0,"  # 下划线(1=启用)
    ...
    try:
        ...
        print("字幕嵌入成功！")
        print("STDOUT:", result.stdout)  # FFmpeg 的标准输出（通常较少）
        print("STDERR:", result.stderr)  # FFmpeg 的错误/日志（主要在这里）
        return True
